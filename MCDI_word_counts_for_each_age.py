# -*- coding: utf-8 -*-
"""MCDI_word_counts_for_each_age.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1IVEN42fFcLfrQNWhfEiC2HDETPBOB9b0

# Code to calculate the word_count without grouping age.
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Calculate WPM and word counts from SRT files in a directory.")
    parser.add_argument("directory_path", type=str, help="Path to the directory containing SRT files to process.")
    
    # Parse the directory path argument
    args = parser.parse_args()
    
    # Access the directory path as a string directly
    directory_path = args.directory_path
    output_excel_path = './MCDI_word_counts_for_each_age.xlsx'
    ...
    for srt_file_path in srt_files:
        ...
        if month:
            ...
            group_stats[month]['total_duration'] += total_duration
            group_stats[month]['mcdi_word_counts'].update(mcdi_word_counts)
            group_stats[month]['all_word_counts'].update(all_word_counts)
            group_stats[month]['unique_words'].update(unique_words)
            group_stats[month]['unique_mcdi_words'].update(unique_mcdi_words)

            for category, words in category_unique_words.items():
                group_stats[month]['category_unique_words'][category].update(words)

            # Update category-specific word counts
            for category, counts in category_word_counts.items():
                group_stats[month]['category_word_counts'][category].update(counts)

    graph_df = pd.DataFrame(columns = ['Month','MCDI over Total','Total MCDI over Unique MCDI'])
    with pd.ExcelWriter(output_excel_path, engine='xlsxwriter') as writer:
        for month, stats in group_stats.items():
            ...
            print("Age Group: ",month,"\n",summary_df)

            new_row = pd.DataFrame({'Month': [month], 'MCDI over Total': [sum(stats['mcdi_word_counts'].values())/sum(stats['all_word_counts'].values()),], 'Total MCDI over Unique MCDI': [len(stats['mcdi_word_counts'].keys())/len(stats['all_word_counts'].keys())]})
            ...
            summary_df.to_excel(writer, sheet_name=f'{month}_Summary', index=False)
            mcdi_counts_df.to_excel(writer, sheet_name=f'{month}_MCDI_Words', index=False)
            all_words_df.to_excel(writer, sheet_name=f'{month}_All_Words', index=False)
